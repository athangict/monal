...
class TAClaimTable extends AbstractTableGateway 
{
	...
	*/
    ...
    // Main query
    $select = $sql->select();
    $select->from(['tc' => $this->table])
           ->join(['a' => 'sys_activity_logs'], 'tc.id = a.process_id')
           ->join(['latest' => $subSelect], 'a.process_id = latest.process_id AND a.id = latest.max_id', [])
           ->columns(['*'])
		   ->order('tc.date DESC'); // gets all columns from ns

    if ($where !== null) {
        $select->where($where);
    }
	$select->where->notEqualTo('tc.author', $user);
	...
	public function getActionByMe($where = NULL,$user)
	{
		...
		$select->from(['tc' => $this->table])
			->join(['a' => 'sys_activity_logs'], 'tc.id = a.process_id',['process_id'])
			->order('tc.date DESC')
			->where->notEqualTo('tc.author', $user);
