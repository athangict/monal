...
class TATable extends AbstractTableGateway 
{
	...
	*/
    ...
    // Main query
    $select = $sql->select();
    $select->from(['ta' => $this->table])
           ->join(['a' => 'sys_activity_logs'], 'ta.id = a.process_id')
           ->join(['latest' => $subSelect], 'a.process_id = latest.process_id AND a.id = latest.max_id', [])
           ->columns(['*'])
		   ->order('ta.date DESC'); // gets all columns from ns

    if ($where !== null) {
        $select->where($where);
    }
	$select->where->notEqualTo('ta.author', $user);
	...
	public function getActionByMe($where = NULL,$user)
	{
		...
		$select->from(['ta' => $this->table])
			->join(['a' => 'sys_activity_logs'], 'ta.id = a.process_id',['process_id'])
			->order('ta.date DESC')
			->where->notEqualTo('ta.author', $user);
