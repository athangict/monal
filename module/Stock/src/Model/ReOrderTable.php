...
class ReOrderTable extends AbstractTableGateway 
{
	protected $table = 'st_reorder'; //tablename
	...
	public function get($param)
	{
		...
		$select->from(array('r' => $this->table))
				->join(array('l' => 'sys_location_type'), 'l.id = r.location_type', array('location_type_id' => 'id', 'location_type_name' => 'location_type'))
		       ->where($where);
	...
	*/
	 ...
	 * @return Array
	 */
	public function getByItemLocationType($item,$location)
	{
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		...
		/*if($request_type != '-1'){
			$select->where(array('request_type'=>$request_type));
		}*/
