...
class TranspInvDetailsTable extends AbstractTableGateway 
{
	protected $table = 'st_transporter_invoice_dtl';   //tablename
	...
	{   
		$where = ( is_array($param) )? $param: array('tr.id' => $param);
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from(array('tr'=>$this->table))
		        ->where($where)
				->order(array('id ASC'));		
		$selectString = $sql->getSqlStringForSqlObject($select);
		$results = $adapter->query($selectString, $adapter::QUERY_MODE_EXECUTE)->toArray();
   ...
   /**
	...
	/**
	 * TRANSPORTATION PAYMENT BOOKING 
