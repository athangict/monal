...
class BatchDetailsTable extends AbstractTableGateway 
{
	protected $table = 'st_batch_details'; //tablename
	...
	public function getSortByLocationName($param)
	{
		...
		$select->from(array('b'=>$this->table))
		      ->join(array('l'=>'adm_location'), 'b.location = l.id', array('location' => 'id', 'location_name' => 'location'))
              ->where($where)
	...
	*/
	...
	public function getLSUM($where=NULL, $column)
	{
		...
		$sub0->columns(array("batch"));
			 
		$adapter = $this->adapter;
		$adapter = $this->adapter;  	 
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from(array('bd'=>$this->table))
			   ->columns(array( new Expression('SUM('.$column.') as total')));
		$select->where->in('batch', $sub0);
		$select->where($where);
		$selectString = $sql->getSqlStringForSqlObject($select);
		//echo $selectString; exit;
		$results = $adapter->query($selectString, $adapter::QUERY_MODE_EXECUTE)->toArray();
	 ...
	 * @return Array
	 */
	public function getLCount($param)
	{
		$sub0 = new Select(array("dd"=>"st_dispatch_details"));
		$sub0->columns(array("batch"));
	...
	public function getCount($param)
	{
        ...
        //region location wise selling price view
	public function getregionlocbatch($param,$location)
	{
		$where = ( is_array($param) )? $param: array('id' => $param);
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from(array('bd'=>$this->table))
				->join(array('l'=>'sys_location'),'l.id=bd.location',array())
				->where($where);
				if($location != '-1'){
