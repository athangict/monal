...
class ItemUomTable extends AbstractTableGateway 
{
	protected $table = 'st_uom_item'; //tablename
	...
	{
		$where = ( is_array($param) )? $param: array('ui.id' => $param);
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from(array('ui'=>$this->table))
	           ->join(array('i'=>'st_items'), 'i.id = ui.item', array('code', 'item_id' => 'id'))
	           ->join(array('u'=>'st_uom'), 'u.id = ui.uom', array('uom_code'=>'code', 'uom_id' => 'id'))
		       ->where($where);
	    $selectString = $sql->getSqlStringForSqlObject($select);
	    $results = $adapter->query($selectString, $adapter::QUERY_MODE_EXECUTE)->toArray();
	    return $results;
