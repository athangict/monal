...
class PriceTable extends AbstractTableGateway 
{
	protected $table = 'st_price'; //tablename
	 ...
	 * @return String | Int
	 */
	public function getMaxID($column, $start_date,$end_date,$where = NULL)
	{
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
	...
	*/
	 ...
	 * @return Array
	 */
	public function getByDate($item,$date)
	{
		//$where = ( is_array($param) )? $param: array('id' => $param);
		...
		$select->from($this->table);	   
			...
			$select->where->between('date', $start_date, $date);
