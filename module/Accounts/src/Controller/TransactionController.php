...
{   
	...
	**/
	public function calculatedAction()
	{
		...
		$nosRate =$form['nos'];
		$weightAMT = $this->getDefinedTable(Accounts\ItemTable::class)->getColumn($item_id,'sdr_weight');
		$nosAMT = $this->getDefinedTable(Accounts\ItemTable::class)->getColumn($item_id,'sdr_nos');
				...
				for($i=0; $i < sizeof($location); $i++):
					if(isset($location[$i]) && is_numeric($location[$i])):
						$tdetailsdata = array(
							...
							'sub_head' => $sub_head[$i],
							'bank_ref_type' => '',
							'cheque_no' =>(!empty($cheque_no[$i]))? $cheque_no[$i]:'DFT-',
	...
	public function creditAction()
	{
		...
		$transTable = $this->getDefinedTable(Accounts\TransactionTable::class)->getDateWiseStatus('voucher_date',$year,$month,$date,$status,$user_region);
		$paginator = new \Laminas\Paginator\Paginator(new \Laminas\Paginator\Adapter\ArrayAdapter($transTable));
	...
	public function addcreditAction()
	{
		...
		return new ViewModel(array(
			...
			'journals' => $this->getDefinedTable(Accounts\JournalTable::class)->getAll(),
			'subheadObj' => $this->getDefinedTable(Accounts\SubheadTable::class),
			'heads' => $this->getDefinedTable(Accounts\HeadTable::class)->getHeads(),
		));
	}
	...
	public function addexpenseAction()
	{
		...
		$loc= $this->getDefinedTable(Accounts\TransactiondetailTable::class)->get(array('transaction' => $application_id));
	...
	public function processrAction()
	{
		...
		if($this->getRequest()->isPost()){	
			...
			$transactiondetailsid=$this->getDefinedTable(Accounts\TransactiondetailTable::class)->get(array('transaction'=>$application_id));
			$app_data = array(
				'id'		=> $application_id,				
			...
			$app_results = $this->getDefinedTable(Accounts\TransactiondetailTable::class)->save($app_data_details);
			endforeach;
			$app_data = $this->_safedataObj->rteSafe($app_data);
			$this->_connection->beginTransaction();
			$app_result = $this->getDefinedTable(Accounts\TransactionTable::class)->save($app_data);
			if($app_result):
				$activity_data = array(
					'process'      => $process_id,
	...
	public function processAction()
	{
		...
		if($this->getRequest()->isPost()){	
			...
			foreach($this->getDefinedTable(Accounts\TransactionTable::class)->get($app_result) as $trans);
		...
		if($this->getRequest()->isPost()){
			...
			if($result > 0){
				...
				if($result1 > 0):
					$this->_connection->commit(); // commit transaction on success
					$this->flashMessenger()->addMessage("success^ New Transaction successfully added | ".$voucher_no);
					return $this->redirect()->toRoute('transaction', array('action' =>'viewagainstdebit', 'id' => $result));
				else:
					$this->_connection->rollback(); // rollback transaction over failure
					$this->flashMessenger()->addMessage("Failed^ Failed to add new Transaction");		
					return $this->redirect()->toRoute('transaction');
	...
	public function editagainstpayrollAction()
	{
		...
		if($this->getRequest()->isPost()){
			...
			$result = $this->getDefinedTable(Accounts\TransactionTable::class)->save($data1);
			if($result > 0){
				foreach($this->getDefinedTable(Accounts\TransactiondetailTable::class)->get(array('transaction'=>$result)) as $dtl):
					$data2 = array(
						'id' => $dtl['id'],
						'transaction' => $result,
						'voucher_dates' =>$form['voucher_date'],
					...
					$result = $this->getDefinedTable(Accounts\TransactiondetailTable::class)->save($data2);
				endforeach;
				$this->_connection->commit(); // commit transaction on success
				$this->flashMessenger()->addMessage("success^ Transaction successfully updated | ".$voucher_no);
				return $this->redirect()->toRoute('transaction', array('action' =>'viewagainstdebit', 'id' => $this->_id));
	...
	public function addiwrAction()
	{
		...
		return new ViewModel(array(
			...
			'journals' => $this->getDefinedTable(Accounts\JournalTable::class)->getAll(),
			'subheadObj' => $this->getDefinedTable(Accounts\SubheadTable::class),
			'heads' => $this->getDefinedTable(Accounts\HeadTable::class)->get(array('h.id'=>$head)),
		));
	}
	...
	public function addiwrreceiptAction()
	{
			...
			if($result > 0){
						...
						if($result1>0):
							/*if(!empty($against_vid[$i])):
								$updatedata1= array(
									'id' => $result,
									'against'   =>1,//Against Id is multiple so not recorded
									'against_vid'  =>$this->getDefinedTable(Accounts\TransactiondetailTable::class)->getColumn($against_vid[$i],'transaction'),//Against Id is multiple so not recorded
									'modified' =>$this->_modified,
								);
								$result2 = $this->getDefinedTable(Accounts\TransactionTable::class)->save($updatedata1);
							endif;*/
						endif;
					endif;
				endfor;//exit;
				...
				for($i=0; $i < sizeof($location); $i++):
					if(isset($location[$i]) && is_numeric($location[$i])):
						...
						$result1 = $this->getDefinedTable(Accounts\CommissionTable::class)->save($commissiondata);
						if($result1 < 0):
							break;
						endif;
					endif;
		...
		return new ViewModel(array(
			...
			'subheadObj' => $this->getDefinedTable(Accounts\SubheadTable::class),
			'heads' => $this->getDefinedTable(Accounts\HeadTable::class)->get(array('h.id'=>$head)),
			'transactions' => $this->getDefinedTable(Accounts\TransactiondetailTable::class)->get(array('voucher_types'=>5,'t.status'=>4,'td.against'=>0)),
		));
	}
	/**
	...
	public function editiwrreceiptAction()
	{
		...
		return new ViewModel(array(
			...
			'heads' => $this->getDefinedTable(Accounts\HeadTable::class)->getAll(),
			'tdetailsObj' => $this->getDefinedTable(Accounts\TransactiondetailTable::class),
			'commissionObj' => $this->getDefinedTable(Accounts\CommissionTable::class)
		));
	}
	/**
	 * view IWR RECEIPT------------------------------------------------------------------------------------------------------
	...
	public function viewiwrreceiptAction(){
		$this->init();
		//$commission = $this->getDefinedTable(Accounts\TransactiondetailTable::class)->get(array('transaction' => $this->_id));
		//foreach(($commission as $ca);
		//$transaction_id=$ca['transaction'];
		$commisamount = $this->getDefinedTable(Accounts\CommissionTable::class)->get(array('transaction'=> $this->_id));
	...
	public function commitiwrreceiptAction(){
		...
		if($result>0){
			...
			foreach($ids as $tasid):
			    ...
			    $results = $this->getDefinedTable(Accounts\TransactiondetailTable::class)->save($data);
		    endforeach;
			$commissionids = $this->getDefinedTable(Accounts\CommissionTable::class)->get(array('transaction'=>$this->_id));
			...
			foreach($commissionids as $cid):
			    ...
			    $results1 = $this->getDefinedTable(Accounts\CommissionTable::class)->save($dataCom);
		    endforeach;
		}
		if($results1):
			foreach($ids as $tasid):
	...
	**/
	public function getvprevAction()
	{
		...
		$loc = '';
		$debit='';
		$tdetails = $this->getDefinedTable(Accounts\TransactiondetailTable::class)->get(array('td.id' => $vd_id));
...
public function getSubheadsAction()
{
  ...
  $sbh = $this->getDefinedTable(Accounts\SubheadTable::class)->get(array('head'=>$head_id));
  foreach($sbh as $sb) {
    $subheads[] = array(
      'value' => $sb['id'],
