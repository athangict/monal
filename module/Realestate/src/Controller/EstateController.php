...
{   
    ...
    public function indexAction()
    {
        ...
        return new ViewModel(array(
            'title' => 'Real Estate',
            'leasedrent' => $this->getDefinedTable(Realestate\LeasedRentTable::class)->getAllwithName(),
        ));
    }
    
    //add leased 
    public function addleasedAction()
    {
        $this->init();
        if($this->getRequest()->isPost())
        {
            ...
            $data = array(
                ...
                'date' => $form['agreement_date'],
                'location' => $form['location'],
                'leased_godown' => $form['leased_property'],
                'lessee' => $form['lessee'],
                'rent' => $form['rent'],
            ...
            return $this->redirect()->toRoute('realestate');
        }
        return new ViewModel(array(
    ...
    //get leased property action
	public function getleasedgodownAction()
	{		
		$form = $this->getRequest()->getPost();
		$location_id = $form['location_id'];
		$location_assets=0;
		$assets = $this->getDefinedTable(Realestate\StorageTypeTable::class)->get(array('location' => $location_id));
		$location_assets.="<option value=''></option>";
		foreach($assets as $ast):
			$location_assets.="<option value='".$ast['id']."'>".$ast['storage']."</option>";
		endforeach;
		
		echo json_encode(array(
			'leaseditem' => $location_assets
		));
		exit;
	}
    //view rents monthly
