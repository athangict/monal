...
class ContractorRunningAmtTable extends AbstractTableGateway 
{
	protected $table = 'rs_contractor_running_amt'; //tablename
	...
	*/
	 ...
	 * @return Array
	 */
	public function getMaxSerial($prefix, $param)
	{  
		//$where = ( is_array($param) )? $param: array('id' => $param);
	    $adapter = $this->adapter;
		$sql = new Sql($adapter);
		...
		$select->from($this->table)
				->where($param);
		$select->where->like('bill_no', $prefix."%");
