...
class RentTable extends AbstractTableGateway 
{
	protected $table = 'rs_rent'; //tablename
	...
	*/
	...
	public function getDistinct($column,$where = NULL)
	{
		...
		$select->from(array('r' => $this->table))
		 		->join(array('l'=>'adm_location'),'l.id=r.location');
	 ...
	 * @return Array
	 */
	public function getRentByMonth($year,$month,$location)
	{
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
