...
class BuildingTable extends AbstractTableGateway 
{
	protected $table = 'rs_building'; //tablename
	...
	*/
	 ...
	 * @return Array
	 */
	public function getReportB($data)
	{
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
	    $select = $sql->select();
		$select->from(array('b'=>$this->table))
		       ->join(array('a'=>'rs_building_master'), 'b.asset = a.id');
		if($data['region'] != '-1'){
			$select->where(array('region'=>$data['region']));
		}
		if($data['location'] != '-1'){
			$select->where(array('a.location'=>$data['location']));
		}
		if($data['block'] != '-1'){
