...
{   
	...
	**/
	public function getheadAction()
	{
		...
		$group_id =$form['group'];
		//$region_id =1;
		$heads = $this->getDefinedTable(Pswf\HeadTable::class)->get(array('h.group' => $group_id));
	...
	{
		$this->init();		
		$min_year = date('Y', strtotime($this->getDefinedTable(Pswf\TransactionTable::class)->getMin('voucher_date')));
		if($this->getRequest()->isPost()):
			...
			if($form['task']=='1'):
				...
				if(!$this->getDefinedTable(Pswf\ClosingbalanceTable::class)->isPresent(array('year'=>$year))):
					foreach($this->getDefinedTable(Pswf\SubheadTable::class)->getAll() as $rows):				
						//$closing_balance = $this->getDefinedTable(Pswf\TransactiondetailTable::class)->getClosingBalance($filter,$data['head'],$rows['id'], 1);
						...
						$this->getDefinedTable(Pswf\ClosingbalanceTable::class)->save($data);
					endforeach;
				endif;
			endif;
		else:
		...
		if($this->getRequest()->isPost()):
            ...
            for($i=0;$i<sizeof($closing_sub_head);$i++){
			  //if($closing_dr[$i] != 0 || $closing_cr[$i] != 0):
				...
				$result = $this->getDefinedTable(Pswf\ClosingbalanceTable::class)->save($data);
              //endif; 				
			if($result > 0 ){
