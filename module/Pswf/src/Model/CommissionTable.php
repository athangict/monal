...
class CommissionTable extends AbstractTableGateway 
{
	protected $table = 'fa_commission'; //tablename
	...
	/**
	 * Return the sum of specific columns based on the given transaction
	 * @param int $transaction
	 * @param string $commission
	 * @param string $decimal
	 * @param string $remitance
	 * @return array
	 */
	public function getSum($transaction, $commission_amt, $decimal_amt, $remitance_amt)
	{
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from($this->table)
			->columns([
				'sumcommission' => new Expression('SUM(' . $commission_amt . ')'),
				'sumdecimal' => new Expression('SUM(' . $decimal_amt . ')'),
				'sumremitance' => new Expression('SUM(' . $remitance_amt . ')'),
			])
	...
	*/
	 ...
	 * @return Array
	 */
	public function getDistinctclass($data)
	{
		$adapter = $this->adapter;
	    $sql = new Sql($adapter);
	    $select = $sql->select();
	    $select->from($this->table)
		       ->columns(array(new Expression('DISTINCT(id) as class')));
	    $selectString = $sql->getSqlStringForSqlObject($select);
		//echo $selectString;exit;
		$results = $adapter->query($selectString, $adapter::QUERY_MODE_EXECUTE)->toArray();
		return $results;
