...
{
	protected $table = 'ps_closing_balance'; //tablename
	...
	public function get($param)
	{
		...
		$select = $sql->select();
		$select->from(array("cb" => $this->table))
	    	   ->join(array("sh"=>"fa_sub_head"),"sh.id=cb.sub_head", array("code"=>"code","name"=>"name","sub_head_id" => "id"))	    
		       ->where($where);
	 ...
	 * @return Array
	 */
	public function getfromgroup($year,$head,$group)
	{
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from(array("cb" => $this->table))
	    	   ->join(array("sh"=>"fa_sub_head"),"sh.id=cb.sub_head", array("sh_code"=>"code","sh_name"=>"name","sub_head_id" => "id"))
			   ->join(array("h"=>"fa_head"),"h.id=cb.head", array("code"=>"code","name"=>"name","head_id" => "id"))
			   ->join(array("g"=>"fa_group"),"g.id=h.group", array("code"=>"code","name"=>"name","group_id" => "id"));
		if($group != '-1'):
			$select->where(array("h.group" => $group));
		endif;
		if($year != '-1'):
			$select->where(array("year" => $year));
		endif;
		if($head != '-1'):
			$select->where(array("cb.head" => $head));
		endif;
		$selectString = $sql->getSqlStringForSqlObject($select);
		//echo $selectString;exit;
		$results = $adapter->query($selectString, $adapter::QUERY_MODE_EXECUTE)->toArray();
