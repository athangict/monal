...
{
	protected $table = 'ps_sub_head'; //tablename
	...
	{  
	    ...
	    $select->from(array('sh'=>$this->table))
				->join(array('h'=>'ps_head'), 'h.id=sh.head', array('head'=>'name', 'head_id'=>'id'))
				->join(array('g'=>'ps_group'), 'g.id=h.group', array('group'=>'name', 'group_id'=>'id'))
				->join(array('c'=>'ps_class'), 'c.id=g.class', array('class'=>'name', 'class_id'=>'id'))
				->join(array('ht'=>'ps_head_type'), 'ht.id=h.head_type', array('head_type', 'headtype_id'=>'id'))
				->join(array('t'=>'ps_type'), 't.id=sh.type', array('type', 'type_id'=>'id', 'type_class'=>'class'))
   ...
   /**
	...
	{		
		...
		$sub = new Select("ps_closing_balance");	
		...
		$sub1 = new Select("ps_transaction_details");
	...
	public function getSubhead($data)
	{
		...
		$select = $sql->select();
	    $select->from(array('sh'=>$this->table))
				->join(array('h'=>'ps_head'), 'h.id=sh.head', array('head'=>'code', 'head_id'=>'id'))
    ...
    {         
            ...
            $select = $sql->select();
            $select->from(array('sh'=>$this->table))
			->join(array('h'=>'ps_head'), 'h.id = sh.head')
			->join(array('ht'=>'ps_head_type'), 'ht.id = h.head_type');
            $select->columns($fetch);
            $select->where($where);
