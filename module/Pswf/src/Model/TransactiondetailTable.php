...
{
	protected $table = 'ps_transaction_details'; 
	...
	{  
	    ...
	    $select->from(array('td'=>$this->table))
			   ->join(array('t'=>'ps_transaction'),'t.id = td.transaction', array('transaction_id' => 'id'))
		       ...
		       ->join(array('h'=>'ps_head'), 'h.id = td.head', array('head' => 'code', 'head_id' => 'id'))
			   ->join(array('sh'=>'ps_sub_head'), 'sh.id = td.sub_head', array('sub_head' => 'code', 'sub_head_id' => 'id'));
	...
	public function get($param)
	{
		...
		$select = $sql->select();
	    $select->from(array('td'=>$this->table))
			   ->join(array('t'=>'ps_transaction'),'t.id = td.transaction', array('transaction_id' => 'id', 'voucher_type'))
			   ->join(array('l'=>'adm_location'), 'l.id = td.location', array('location', 'location_id' => 'id'))
			   //->join(array('a'=>'adm_activity'), 'a.id = td.activity', array('activity', 'activity_id' => 'id'))
			   ->join(array('h'=>'ps_head'), 'h.id = td.head', array('head' => 'code', 'head_id' => 'id'))
			   ->join(array('sh'=>'ps_sub_head'), 'sh.id = td.sub_head', array('sub_head' => 'code', 'sub_head_name' => 'name','sub_head_id' => 'id'))
		       ->where($where);
	 ...
	 * @return Array
	 */
	public function getforreconcile($location,$start_date,$end_date,$reconcile,$bank)
	{
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
	    $select->from(array('td'=>$this->table))
			   ->join(array('l'=>'adm_location'), 'l.id = td.location', array('location', 'location_id' => 'id'));
		$select->where(array("reconcile" =>$reconcile))
			   ->where->between('voucher_dates',$start_date,$end_date);
		if($bank != '-1'):
			$select->where(array("sub_head" => $bank));
		endif;
		if($location != '-1'):
			$select->where(array("l.id" => $location));
		endif;
		$selectString = $sql->getSqlStringForSqlObject($select);
		//echo $sbh;exit;
		$results = $adapter->query($selectString, $adapter::QUERY_MODE_EXECUTE)->toArray();
	...
	*/
	...
	public function getMaxRow($column,$param)
	{
		...
		$select->from(array('td'=>$this->table))
			   ...
			   ->join(array('h'=>'ps_head'), 'h.id = td.head', array('head' => 'code', 'head_id' => 'id'))
			   ->join(array('sh'=>'ps_sub_head'), 'sh.id = td.sub_head', array('sub_head' => 'code', 'sub_head_id' => 'id'))
			   ->join(array('brt'=>'ps_bank_ref_type'), 'brt.id = td.bank_ref_type', array('bank_ref_type', 'bank_ref_type_id' => 'id'))
			   ->where($where)
			   ->where($column);
	...
	{	
		...
		$select->from($this->table)
			   ...
			   ->where->between('voucher_dates', $start_date, $end_date);
		$sub1 = new Select("ps_group");
		$sub1->columns(array("id"))
			->where(array("class" => $class));
		
		$sub2 = new Select("ps_head");
	...
	public function getSumbyHead($activity,$region,$location,$start_date,$end_date, $column, $head)
	{
		...
		$select->from(array('t'=>$this->table))
		       ->join(array('h'=>'ps_head'), 'h.id=t.head', array('head'=>'name', 'head_id'=>'id'));
	...
	*/
	 ...
	 * @return Int
	 */
	public function getClosingBalanceforPresBS($activity,$region,$location,$starting_date,$ending_date, $id, $tier)
	{	
	...
	{		
		$sub0 = new Select("ps_transaction");
	 ...
	 * @return Int
	 */
	public function getOpeningBalanceforSHLedger($start_date,$id)
	{	
		$adapter = $this->adapter;  
		$sql = new Sql($adapter);
		$year = date('Y', strtotime($start_date));
	...
	{		
		...
		$select = $sql->select();
		$select->from(array('td'=>$this->table))
			->join(array('t'=>'ps_transaction'),'t.id = td.transaction', array('cheque_no','voucher_no','voucher_date','status','remark'));
	...
	public function getCash($voucher_type,$opening_date,$closing_date,$where)
	{
		...
		$select = $sql->select();
		$select->from(array('td'=>$this->table))
				->join(array('t'=>'ps_transaction'),'t.id = td.transaction', array('voucher_type','voucher_no','voucher_date','status','remark'))
		        ->where($where)
	...
	public function getCashBookHead($param)
	{
		...
		$select = $sql->select();
		$select->from(array('td'=>$this->table))
				->join(array('h'=>'ps_head'), 'h.id = td.head', array('head' => 'code'));
		$select->columns(array(
	...
	{	//print_r($options); echo $tier; exit; 
		...
		$sub0 = new Select("ps_closing_balance");
	...
	public function getSumbyHeadforPresPLSA($activity,$region,$location,$starting_date,$ending_date,$column, $head)
	{
		...
		$select->from(array('t'=>$this->table))
		       ->join(array('h'=>'ps_head'), 'h.id=t.head', array('head'=>'name', 'head_id'=>'id'))
			   ->join(array('fa'=>'ps_head_format_audit'), 'fa.id=h.format_audit', array('name', 'format_audit_id'=>'id'));
		$select ->columns(array( new Expression('SUM('.$column.') as total')))
	...
	public function getSumbyHeadforPrevPLSA($activity,$region,$location,$starting_date,$ending_date,$column, $head)
	{
		...
		$select->from(array('t'=>$this->table))
		       ->join(array('h'=>'ps_head'), 'h.id=t.head', array('head'=>'name', 'head_id'=>'id'))
			   ->join(array('ht'=>'ps_head_format'), 'ht.id=h.format', array('name', 'format_id'=>'id'))
			   ->join(array('fa'=>'ps_head_format_audit'), 'fa.id=h.format_audit', array('name', 'format_audit_id'=>'id'));
		$select->columns(array( new Expression('SUM('.$column.') as total')))
	...
	{  
	    ...
	    $select->from(array("bt" => $this->table))
	    	   ->join(array("h"=>"ps_head"),"h.id=bt.head", array("head"=>"code","head_id" => "id"))
			   ->join(array("g"=>"ps_group"),"g.id=h.group", array("group"=>"code","group_id" => "id"))
			   ->join(array("c"=>"ps_class"),"c.id=g.class", array("class"=>"code","class_id" => "id"))
