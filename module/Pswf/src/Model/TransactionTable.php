...
{
	protected $table = 'ps_transaction'; //tablename
	...
	public function getVouchersPresent($start_date,$closing_date,$location_id,$head_id)
	{
		...
		$select = $sql->select();
		$select->from(array('t'=>$this->table))
				->join(array('td'=>'ps_transaction_details'),'t.id = td.transaction', array('location', 'head'))
	...
	*/ 
	public function getDisinctTransactionBAID($bank_account,$start_date,$end_date)
	{
		...
		$select = $sql->select();
		$select->from(array('t'=>$this->table))
			   ->join(array('td'=>'ps_transaction_details'),'t.id = td.transaction', array())
	...
	*/
	...
	{	
		...
		$select = $sql->select();    
		$select->from(array('t'=>$this->table))
				->join(array('td'=>'ps_transaction_details'),'t.id = td.transaction', array('head','debit','credit'))
				//->join(array('j'=>'ps_journal'),'j.id = t.voucher_type', array('voucher_type'=>'journal','voucher_id' => 'id'))
				->join(array('sh'=>'ps_sub_head'),'sh.id = td.sub_head', array('code'))
                 ->columns(array(
	...
	*/ 
	public function getVoucherChecking($data)
	{
		...
		$select = $sql->select();
		$select->from(array('t'=>$this->table))
			->join(array('td'=>'ps_transaction_details'),'t.id = td.transaction', array('location'));
			$select->columns(array(
