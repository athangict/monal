...
class TransportCashMemoTable extends AbstractTableGateway 
{
	protected $table = 'tp_cash_memo'; //tablename
	...
	public function get($param)
	{
		...
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from(array('cm' => $this->table))
			        ->join(array('so'=>'tp_sanction_order'),'so.id = cm.sanction_order', array('sanction_order_id' => 'id'))
			    ->where($where)
	    	   ->order(array('id ASC'));
	...
	*/
	 ...
	 * @return Array
	 */
	public function getMonthlyCM($prefix_PO_code)
	{  
		$adapter = $this->adapter;			
		$sql = new Sql($adapter);		
		$select = $sql->select();
		$select->from($this->table);		
		$select->where->like('cash_memo_no', $prefix_PO_code."%");	
	 ...
	 * 
	 **/
	public function getCashMemo($start_date,$end_date,$where)
	{		
