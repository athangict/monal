...
class VehiclePartTable extends AbstractTableGateway 
{
	protected $table = 'tp_vehicle_parts';   //tablename
	...
	{
		$where = ( is_array($param) )? $param: array('vp.id' => $param);
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from(array('vp' => $this->table))
		       	->join(array('u'=>'st_uom'), 'u.id = vp.uom', array('st_uom_code'=>'code', 'st_uom_id' => 'id'))
			   ->where($where);
