...
class ReceiptDtlsTable extends AbstractTableGateway 
{
	protected $table = 'sl_receipt_dtls'; //tablename
	...
	{  
	    ...
	    $sql = new Sql($adapter);
	    $select = $sql->select();
	    $select->from(array('rd'=>$this->table))
	           ->join(array('r'=>'sl_receipt'), 'r.id = rd.receipt', array('receipt_no', 'receipt_id'=>'id'))
	     	   ->join(array('s'=>'sl_sales'), 's.id = rd.sales', array('sales_id'=>'id', 'sales_no'))
			   ->order(array('id DESC'));
	...
	{
		$where = ( is_array($param) )? $param: array('rd.id' => $param);
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from(array('rd'=>$this->table))
