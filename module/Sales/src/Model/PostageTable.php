...
class PostageTable extends AbstractTableGateway 
{
	protected $table = 'sl_postage'; //tablename
	...
	*/
	public function isPresent($column, $value)
	{
		...
		$resultSet = $this->select(function(Select $select) use ($column, $value){
			$select->where(array($column => $value,'status'=>[2,4]));
		});
	...
	**/ 
	...
	//Get Service  
	public function getServiceSales($data,$start_date,$end_date, $where = NULL)
	{
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		...
		if($data['scope'] != '-1'){
			$select->where(array('scope'=>$data['scope']));
		}
		if($data['service'] != '-1'){
			$select->where(array('service'=>$data['service']));
		}
		if($data['status'] != '-1'){
			$select->where(array('status'=>$data['status']));
		}
		if($where!=NULL){
			$select->where($where);
		}
	...
	//Get Service  
	public function getServiceReportByUsers($data,$start_date,$end_date, $where = NULL)
	{
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		...
		if($data['user'] != '-1'){
			$select->where(array('author'=>$data['user']));
		}
		if($data['location'] != '-1'){
			$select->where(array('location'=>$data['location']));
		}
		$select->where(array('status' =>4));
		if($where!=NULL){
			$select->where($where);
		}
		$selectString = $sql->getSqlStringForSqlObject($select);
	 ...
	 */
	public function getSum($column, $data, $start_date, $end_date, $where = NULL)
	{
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
