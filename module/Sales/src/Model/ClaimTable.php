...
class ClaimTable extends AbstractTableGateway 
{
	protected $table = 'sl_claims'; //tablename
	...
	*/
	 ...
	 * @return Array
	 */
	public function getMonthlyCL($prefix_CL_code)
	{  
		$adapter = $this->adapter;			
		$sql = new Sql($adapter);		
		$select = $sql->select();
		$select->from($this->table);		
		$select->where->like('claim_no', $prefix_CL_code."%");	
