...
class SchemeBatchTable extends AbstractTableGateway 
{
	protected $table = 'sl_scheme_batch'; //tablename
	...
	*/
	 ...
	 * @return Array
	 */
	public function getByBatchScheme($batch, $scheme, $location)
	{		
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		...
		if($batch != '-1'){
			$select->where(array('batch'=>$batch));
		}
		if($scheme != '-1'){
			$select->where(array('scheme'=>$scheme));
		}
		if($location != '-1'){
