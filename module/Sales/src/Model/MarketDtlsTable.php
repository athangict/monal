...
class MarketDtlsTable extends AbstractTableGateway 
{
	protected $table = 'sl_market_rate_dtls'; //tablename
	...
	{  
	    ...
	    $sql = new Sql($adapter);
	    $select = $sql->select();
	    $select->from(array('mrd'=>$this->table))
	           ->join(array('mr'=>'sl_market_rate'), 'mr.id = mrd.market', array('document_no', 'market_id'=>'id'))
	           ->join(array('i'=>'st_items'), 'i.id = mrd.item', array('name', 'item_id'=>'id'))
			   	->join(array('u'=>'st_uom'), 'u.id = mrd.uom', array('uom_code'=>'code', 'uom_id' => 'id'))
			   ->order(array('id DESC'));
	...
	{
		$where = ( is_array($param) )? $param: array('mrd.id' => $param);
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from(array('mrd'=>$this->table))
