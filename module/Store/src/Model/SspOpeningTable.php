...
class SspOpeningTable extends AbstractTableGateway 
{
	protected $table = 'in_opening_storespare'; //tablename
	...
	*/
	...
	{	
		...
		$select->from(array('b'=>$this->table))
                 ->columns(array(
					'id','storespare','item','uom','quantity','opening_date','status','author','created','modified',
	 ...
	 * @return Array
	 */
	public function filterSRSsp($start_date, $location, $param, $col_date, $col_loc,$array,$item_sub_group,$item)
	{
		...
		$select->from(array('o'=>$this->table))
			   ->join(array('od'=>'in_opening_storespare_dtls'), 'o.id = od.storespare', array())
			   ->join(array('ssp'=>'in_storespare'), 'o.storespare = ssp.storespare', array())
	...
	**/ 
	 ...
	 * @return SUM
	**/ 
	public function fetchSROpeningSUMATM($start_date, $location, $param)
	{   
		$where = ( is_array($param) )? $param: array('id' => $param);
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		...
		$select->from(array('o' => $this->table))
			   ->join(array('od'=>'in_opening_storespare_dtls'), 'o.id = od.storespare', array());
				$select->columns(array(
					'sum' => new Expression('SUM(od.amount)')
			    ));
				$select->where($where);
				$select->where->lessThanOrEqualTo('o.opening_date',$start_date);
				if($location != '-1'){
	 ...
	 * @return SUM
	**/ 
	public function fetchSROpeningSUMRt($start_date, $location, $param)
	{   
		$where = ( is_array($param) )? $param: array('id' => $param);
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		...
		$select->from(array('o' => $this->table))
			   ->join(array('od'=>'in_opening_storespare_dtls'), 'o.id = od.storespare', array());
				$select->columns(array(
					'sum' => new Expression('SUM(od.rate)')
			    ));
				$select->where($where);
				$select->where->lessThanOrEqualTo('o.opening_date',$start_date);
				if($location != '-1'){
