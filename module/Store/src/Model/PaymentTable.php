...
{
	protected $table = 'in_purchase_payment';   //tablename
	...
	{	
		...
		$select->from($this->table)
                 ->columns(array(
					'id','payment_no','payment_date','location','supplier','cost_center','item_group','acc_type','bank_ref_type','cheque_no','payment_amount',
					'bank_charge','transaction','note','status','author','created','modified',
	 ...
	 * @return Array
	 */
	public function getMonthlyPayment($date)
	{
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from($this->table);
		$select->where->like('payment_no', "%".$date."%");
