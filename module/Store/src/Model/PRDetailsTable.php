...
class PRDetailsTable extends AbstractTableGateway 
{
	protected $table = 'in_purchase_receipt_dtls';   //tablename
	 ...
	 * @return Array
	*/ 
	public function getPRDetails($start_date, $end_date, $param, $column)
	{   
		...
		$select = $sql->select();
		$select->from(array('d'=>$this->table))			 
			   ->join(array('pr' => 'pur_purchase_receipt'), 'pr.id = d.purchase_receipt', array('purchase_receipt_no','purchase_receipt_date','purchase_order','location','supplier','cost_center','challan_no','challan_date','note'))
			   ->where($param)
			   ->order(array('id DESC'))			
   ...
   /**
	 ...
	 * @return Array
	 */
	public function getBySupDate($location, $supplier, $start_date, $end_date)
	{		
		$status = array(3);
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		...
		$select->from(array('prn'=>$this->table))
		       ->join(array('pr'=>'pur_purchase_receipt'), 'prn.purchase_receipt = pr.id', array('prn_date', 'challan_no', 'location', 'status', 'purchase_order'))
	           ->join(array('i'=>'st_items'), 'prn.item = i.id', array('item_id' => 'id', 'supplier'));
	      
	    if($location != -1):
			$select->where(array('pr.location'=>$location));
		endif;
		if($supplier != ""):
			$select->where(array('i.supplier'=>$supplier));
		endif;
		$select->where->between('pr.prn_date', $start_date, $end_date)
	...
	{	
		...
		$select->from(array('prd'=>$this->table))
	           ...
	           ->join(array('sg'=>'in_item_subgroup'), 'i.item_sub_group = sg.id', array())
			   ->columns(array(
					'sum' => new Expression('SUM(prd.sound_quantity*prd.rate)')
			    ));
	...
	public function getSUM($start_date,$end_date,$location,$param, $column)
	{
		...
		$select->from(array('pd'=>$this->table))
		       ->join(array('p'=>'in_purchase_receipt'),'p.id=pd.purchase_receipt',array());
		$select->columns(array(
			'sum' => new Expression('SUM('.$column.')')
		));
		$select->where->between('p.purchase_receipt_date',$start_date,$end_date);
		if($location != 0):
		$select->where->equalTo('p.location',$location);
		endif;
		$select->where($where);
		$selectString = $sql->getSqlStringForSqlObject($select);
		//echo $selectString; exit;
	...
	public function getSUMAmt($end_date,$location,$param, $column)
	{
		...
		$select->where->lessThanOrEqualTo('p.purchase_receipt_date',$end_date);
		if($location != 0):
		$select->where->equalTo('p.location',$location);
		endif;
		$select->where($where);
	 ...
	 * @return String | Int
	 */
	public function getPRate($start_date,$end_date,$location,$param, $column)
	{
		$where = ( is_array($param) )? $param: array($param);
		...
		$select->from(array('pd'=>$this->table))
		       ->join(array('p'=>'in_purchase_receipt'),'p.id=pd.purchase_receipt',array());
	    $select->columns(array('rate' =>$column));
		$select ->Limit(1);
	    $select->order(array('pd.id DESC'));
		$select->where->between('p.purchase_receipt_date',$start_date,$end_date);
		if($location != 0):
		$select->where->equalTo('p.location',$location);
		endif;
