...
class SspOpeningDetailsTable extends AbstractTableGateway 
{
	protected $table = 'in_opening_storespare_dtls'; //tablename
	...
	*/
	...
	public function getSUM($where=NULL, $column)
	{
		...
		$select->from(array('sd'=>$this->table))
		       ->join(array('op'=>'in_opening_storespare'),'op.id=sd.storespare',array());
		$select->columns(array(
				'sum' => new Expression('SUM('.$column.')')
		));
		if($where!=NULL){
			$select->where($where);
			$select->where->greaterThan('sd.quantity','0');
		}
		$selectString = $sql->getSqlStringForSqlObject($select);
		//echo $selectString; exit;
		$results = $adapter->query($selectString, $adapter::QUERY_MODE_EXECUTE)->toArray();
